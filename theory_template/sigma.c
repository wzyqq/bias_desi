...
void getwp(double *param, double *w_analysis)
{
	...
	int location;
	int high_limit;
	double *xi_s, *xi_more_s;
	double error,bias;
	double phoxi[node_pho]={0};
	double phowi[node_pho]={0};
	double pho_morexi[node_pho_more]={0};
	double pho_morewi[node_pho_more]={0};	
	double *w1, *w2, *w3, *w4, *w;	
	double *corr_y, *corr_more_y;
	...
	gsl_integration_glfixed_table *table_pho_more;

	table_pho 		= gsl_integration_glfixed_table_alloc(node_pho);
	table_pho_more	= gsl_integration_glfixed_table_alloc(node_pho_more);
...
//		loc_error_distance_low);
	...
	xi_more_s	= (double *)malloc(node_pho*Perpendicular_min*sizeof(double));
	w1  		= (double *)malloc(wp_number*Perpendicular_min*sizeof(double));
	w2   		= (double *)malloc(wp_number*Perpendicular_min*sizeof(double));
	w3  		= (double *)malloc(wp_number*Perpendicular_min*sizeof(double));
	w4   		= (double *)malloc(wp_number*Perpendicular_min*sizeof(double));	
	w    		= (double *)malloc(wp_number*Perpendicular_min*sizeof(double));
    corr_y 		= (double *)malloc(node_pho*sizeof(double));
    corr_more_y = (double *)malloc(node_pho*sizeof(double));
...
//			printf("%lf\t%lf\n", phoxi[i], phowi[i]);
		...
		for(i=0; i<node_pho_more; i++)
		{
			gsl_integration_glfixed_point(high_limit,4*error_distance,i,&pho_morexi[i],&pho_morewi[i],table_pho_more);
//			printf("%lf\t%lf\n", phoxi[i], phowi[i]);
		}
...
//				printf("xi_s[%d][%d]=%lf\n", pe, pa, xi_s[pe*node_pho+pa] );
			...
			for(pa = 0; pa<node_pho_more; pa++)		
			{	
				location=approximation(Perpendicular[pe],pho_morexi[pa]);
				xi_more_s[pe*node_pho_more+pa]=Corr_mm[location];
//				printf("xi_s[%d][%d]=%lf\n", pe, pa, xi_s[pe*node_pho+pa] );
			}			
		}    
...
//				printf("corry[%d]=%lf\n", s, corr_y[s]);
			...
			for(s=0; s<node_pho_more; s++)
			{
				corr_more_y[s] = xi_more_s[j*node_pho_more+s];
//				printf("corry[%d]=%lf\n", s, corr_y[s]);
			}			
			...
			gsl_spline *spline10 = gsl_spline_alloc (gsl_interp_cspline, node_pho_more);
			gsl_spline_init (spline10, pho_morexi, corr_more_y, node_pho_more);
...
//				printf("node=%d\tw2[%d][%d]=%lf\n", i, num, j, w2[num*Perpendicular_min+j]);
			...
			for(i=0; i<node_pho_more; i++)
			{
				w3[num*Perpendicular_min+j] = w3[num*Perpendicular_min+j]+gsl_spline_eval(spline10, pho_morexi[i], acc10)*gsl_sf_erf( (high_limit+pho_morexi[i])/sqrt(2)/error_distance )*pho_morewi[i];
//				printf("node=%d\tw3[%d][%d]=%lf\n", i, num, j, w3[num*Perpendicular_min+j]);
			}

			for(i=0; i<node_pho_more; i++)
			{
				w4[num*Perpendicular_min+j] = w4[num*Perpendicular_min+j]+gsl_spline_eval(spline10, pho_morexi[i], acc10)*gsl_sf_erf( (-high_limit+pho_morexi[i])/sqrt(2)/error_distance )*pho_morewi[i];
//				printf("node=%d\tw4[%d][%d]=%lf\n", i, num, j, w4[num*Perpendicular_min+j]);
			}

			w[num*Perpendicular_min+j] = w1[num*Perpendicular_min+j] + w2[num*Perpendicular_min+j]
			+ w3[num*Perpendicular_min+j] - w4[num*Perpendicular_min+j]; 		
			printf("w[%d][%d]=%lf\t%lf\t%lf\t%lf\t%lf\n", num, j, w1[num*Perpendicular_min+j], w2[num*Perpendicular_min+j], 
				w3[num*Perpendicular_min+j], w4[num*Perpendicular_min+j], w[num*Perpendicular_min+j]);
...
//	printf("error_distance=%lf\n", error_distance);
	...
	xi_s = (double *)malloc(node_pho*Perpendicular_min*sizeof(double));
	w1   = (double *)malloc(wp_real_number*Perpendicular_min*sizeof(double));
	w2   = (double *)malloc(wp_real_number*Perpendicular_min*sizeof(double));
	w    = (double *)malloc(wp_real_number*Perpendicular_min*sizeof(double));
    corr_y = (double *)malloc(node_pho*sizeof(double));

	for(i=0; i<wp_real_number*Perpendicular_min; i++)	
	{ 
		w1[i]=0;
		w2[i]=0;
		w[i]=0;
...
//				printf("corry[%d]=%lf\n", s, corr_y[s]);
			...
			for(i=0; i<node_pho; i++)		
			{ 	
				w[num*Perpendicular_min+j] = w[num*Perpendicular_min+j]+gsl_spline_eval(spline9, phoxi[i], acc9)*phowi[i]*2;
//				w2[num][j] = w2[num][j]+corr_2d[i][j]*2;
//				printf("node=%d\tw1[%d][%d]=%lf\n", i, num, j, w1[num*Perpendicular_min+j]);
	...
	for(i=0; i<wp_real_number*Perpendicular_min; i++)
	{
		w_analysis_real[i] = bias*bias*w[i];
//		printf("w[%d]=%lf\tw_analysis[%d]=%lf\n", i, w[i], i, w_analysis[i]);		
	}
